...
{
  options = {
    home.fileReconciliation.exclude = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ ];
      description = ''
        Exact HOME-relative targets excluded from the global reconciliation
        default. These remain symlink-managed unless explicitly opted in.
      '';
    };
    home.fileReconciliation.enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
        Default to writable reconciliation for managed files, including files
        generated by modules and the XDG file options. Individual files can set
        reconciliation.enable = false to retain symlink management.
        This does not expand supported file types: directories, non-text files,
        recursive files, and force-managed files require explicit opt-outs.
        Unsupported files fail rather than silently falling back to symlinks.
      '';
    };
